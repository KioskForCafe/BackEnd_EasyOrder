...
public class AnalysisController {
    ...
    @ApiOperation(value = "기간동안의 매장 매출 정보 조회", notes = "Path Variable에 매장 번호와 매출인식 시작일, 매출인식 마감일을 포함하여 요청하면, 성공 시 기간동안의 매출액, 판매 건수, 건당 평균 매출 금액 데이터를 반환, 실패 시 실패 메세지를 반환")
    @GetMapping(GET_SALE)
    public ResponseDto<GetAnalysisSaleResponseDto> getAnalysisSale(
        ...
        @PathVariable("endedAt") String endedAt
            ...
            ResponseDto<GetAnalysisSaleResponseDto> response = analysisService.getAnalysisSale(storeId, startedAt, endedAt);
            return response;
    }

    @ApiOperation(value = "매장 시간대별 매출 정보 조회", notes = "Path Variable에 매장 번호와 매출인식 시작일, 매출인식 마감일을 포함하여 요청하면, 성공 시 기간동안의 매출액의 시간대별 데이터(리스트)를 반환, 실패 시 실패 메세지를 반환")
    ...
    public ResponseDto<List<GetAnalysisBusinessResponseDto>> getAnalysisBusiness(
        ...
        ResponseDto<List<GetAnalysisBusinessResponseDto>> response = analysisService.getAnalysisBusiness(storeId, startedAt, endedAt);
        return response; 
    }

    @ApiOperation(value = "매장 상품별 매출 정보 조회", notes = "Path Variable에 매장 번호와매출인식 시작일, 매출인식 마감일을 포함하여 요청하면, 성공 시 상품별, 카테고리별 판매 건수와 금액 데이터를 반환, 실패 시 실패 메세지를 반환")
    @GetMapping(GET_BUSINESS)
    public ResponseDto<GetAnalysisMenuResponseDto> getAnalysisMenu(
        ...
        ResponseDto<GetAnalysisMenuResponseDto> response = analysisService.getAnalysisMenu(storeId, startedAt, endedAt);
        return response;
    }

    @ApiOperation(value = "매장 회원 정보 조회", notes = "Path Variable에 매장 번호와 매출인식 시작일, 매출인식 마감일을 포함하여 요청하면, 총 회원수, 신규 회원, 회원 평균 방문 수, Top 10 회원 정보(순위, 전화번호, 방문수, 적립 포인트, 결제 금액)의 데이터를 반환, 실패 시 실패 메세지를 반환")
    @GetMapping(GET_BUSINESS)
    public ResponseDto<GetAnalysisUserResponseDto> getAnalysisUser(
        ...
        ResponseDto<GetAnalysisUserResponseDto> response = analysisService.getAnalysisUser(storeId, startedAt, endedAt);
        return response;
    }
}
