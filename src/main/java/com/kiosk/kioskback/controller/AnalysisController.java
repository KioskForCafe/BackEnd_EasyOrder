...
public class AnalysisController {
    ...
    @Operation(summary = "기간동안의 매장 매출 정보 조회", description = "Request Header Athorization에 Bearer JWT를 포함하고 Path Variable에 매장 번호와 매출인식 시작일, 매출인식 마감일을 포함하여 요청하면, 성공 시 기간동안의 매출액, 판매 건수, 건당 평균 매출 금액 데이터를 반환, 실패 시 실패 메세지를 반환")
    @GetMapping(GET_SALE)
    public ResponseDto<GetAnalysisSaleResponseDto> getAnalysisSale(
        @AuthenticationPrincipal String userId,
        @Parameter(description = "매장 번호", example = "1", required = true)
        @PathVariable("storeId") int storeId, 
        @Parameter(description = "매출인식 시작일", example = "2023-04-27", required = true)
        @PathVariable("startedAt") @DateTimeFormat(pattern = "yyyy-MM-dd") Date startedAt,
        @Parameter(description = "매출인식 마감일", example = "2023-04-27", required = true)
    ...
    @Operation(summary = "매장 시간대별 매출 정보 조회", description = "Request Header Athorization에 Bearer JWT를 포함하고 Path Variable에 매장 번호와 매출인식 시작일, 매출인식 마감일을 포함하여 요청하면, 성공 시 기간동안의 매출액의 시간대별 데이터(리스트)를 반환, 실패 시 실패 메세지를 반환")
    ...
    @Operation(summary = "매장 상품별,카테고리별 매출 분석 정보 조회", description = "Request Header Athorization에 Bearer JWT를 포함하고 Path Variable에 매장 번호와매출인식 시작일, 매출인식 마감일을 포함하여 요청하면, 성공 시 상품별, 카테고리별 판매 건수와 금액 데이터를 반환, 실패 시 실패 메세지를 반환")
    @GetMapping(GET_MENU)
    ...
    @Operation(summary = "매장별 회원 분석 정보 조회", description = "Request Header Athorization에 Bearer JWT를 포함하고 Path Variable에 매장 번호와 매출인식 시작일, 매출인식 마감일을 포함하여 요청하면, 총 회원수, 신규 회원, 회원 평균 방문 수, Top 10 회원 정보(순위, 전화번호, 방문수, 적립 포인트, 결제 금액)의 데이터를 반환, 실패 시 실패 메세지를 반환")
    @GetMapping(GET_USER)
